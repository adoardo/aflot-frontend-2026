          ...
          <div class="container">
            ...
            <div class="resume-main">
              ...
              <div class="resume-avatar">
                ...
                />
                <img class="avatar" v-if="formData.photo_path" style="position: absolute" :src="createObjectURL(formData.photo_path)" alt="Аватар">
                <span v-else>{{ formData.last_name.substring(0, 1).toUpperCase() }} {{ formData.first_name.substring(0, 1).toUpperCase() }}</span>
                <button @click="changeAvatar()" class="change-av-button header-menu__button" style="bottom: -18px !important;">Выбрать изображение</button>
                <button v-if="formData.photo_path" class="delete" @click="deleteAvatar()">Удалить изображение</button>
              </div>
            ...
            <div class="files">
              <div class="file">
                <ul v-if="uploadedFiles.length" class="header-menu__dopnav">
                  <tr v-for="(item, index) in uploadedFiles" :key="index">
                    <td data-title="Период работы">
                      <NuxtLink style="font-size: 14px;">{{ item.name || getFileName(item) }}</NuxtLink>
                    </td>
                    <td class="resume-experience__delete">
...
const saveResume = async () => {
  ...
  await Promise.all(FILE_KEYS.map(async fileKey => {
    ...
    if(fileKey === 'media_files') {
      await Promise.all(dataToSubmit['media_files'].map( async (file, i)  => {
